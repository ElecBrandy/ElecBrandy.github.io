...
### step 05
과제를 해결하기 위해 중요한 개념을 다시 정리해보자.  

라우터와 스위치는 모두 네트워크 환경에서 중요한 역할을 수행하지만, 각각의 역할과 기능이 상이하다. 이들의 주요 역할을 명확히 이해하는 것은 네트워크 설계와 관리에 필수적이다.  

**라우터**는 네트워크 계층(레이어 3)에서 작동하며, 서로 다른 네트워크 간의 데이터 패킷을 전송하는 장치이다. 라우터는 IP 주소와 서브넷 마스크를 기반으로 데이터 패킷의 목적지를 결정하고, 가장 효율적인 경로로 패킷을 전송한다. 따라서, 라우터를 통해 연결된 각 네트워크는 **고유의 서브넷 마스크와 IP 주소 범위를 가져야** 한다. 이를 통해 라우터는 각 네트워크를 명확히 구분하고, 올바른 네트워크로 패킷을 라우팅할 수 있다.  

**스위치**는 데이터 링크 계층(레이어 2)에서 작동하며, 하나의 네트워크 내에서 여러 기기들 간의 데이터 전송을 담당하는 장치이다. 스위치는 연결된 기기들 간에 데이터 프레임을 전달한다. 이 프레임은 네트워크 내의 특정 기기로 정확하게 전달되어야 한다. 이 과정에서 스위치는 각 기기의 물리적 주소인 MAC 주소를 이용하여 데이터 프레임의 목적지를 찾는다. 스위치로 연결된 모든 기기들은 **동일한 네트워크에 속해 있으므로, 같은 IP 주소 범위를 공유**한다. 
