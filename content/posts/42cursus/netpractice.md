...
### step 04
새로운 서브넷 마스크 `255.255.255.192`에 다른 서브넷 구조는 다음과 같다. 이때 `2^6` 즉 할당받은 범위는 64이지만 호스트 부분이 모두 0인경우 네트워크ID 주소, 호스트 부분이 모두 1인 경우 브로드캐스트 주소로 쓰이므로 일반적으로 할당에서 제외된다.  
...
### step 05
결론적으로 서브넷 마스크는 주어진 IP주소 네트워크 부분과 호스트 부분을 구분하는 데 사용된다. 이 구분은 주로 네트워크 관리를 목적으로 하며 서브넷마스크를 변경함으로써 네트워크 구조를 변경할 수 있다.
